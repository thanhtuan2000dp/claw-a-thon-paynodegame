...
- **LLM** (`core/llm.py`) — OpenAI-compatible (GreenNode MaaS) via `langchain_openai.ChatOpenAI`. `complete_json()` tolerates ```json fences, surrounding prose, and `<think>` reasoning blocks (Qwen gets `/no_think` auto-appended). Use it rather than parsing model output by hand.
